...
import { Sessao } from '@/types'; // Importar o tipo Sessao
...
interface ContinuousCalendarProps {
  ...
  sessoes?: Sessao[]; // Nova propriedade para receber as sessões
}

export const ContinuousCalendar: React.FC<ContinuousCalendarProps> = ({ onClick, onAddEventClick, sessoes = [] }) => {
  ...
  // Agrupa as sessões por data para fácil acesso
  const sessoesPorData = useMemo(() => {
    const mapa = new Map<string, Sessao[]>();
    for (const sessao of sessoes) {
      const dataKey = format(new Date(sessao.dataHoraInicio), 'yyyy-MM-dd');
      if (!mapa.has(dataKey)) {
        mapa.set(dataKey, []);
      }
      mapa.get(dataKey)!.push(sessao);
    }
    return mapa;
  }, [sessoes]);
    ...
    const calendar = calendarWeeks.map((week, weekIndex) => (
      <div className="flex w-full" key={`week-${weekIndex}`}>
        {week.map(({ month, day }, dayIndex) => {
          ...
          const dataKey = format(new Date(year, month, day), 'yyyy-MM-dd');
          const sessoesDoDia = sessoesPorData.get(dataKey) || [];
          const tooltipText = sessoesDoDia.map(s => `${format(new Date(s.dataHoraInicio), 'HH:mm')} - ${s.nome}`).join('\n');
          ...
          return (
            <div
              ...
              title={tooltipText}
              className={`relative z-10 m-[-0.5px] group aspect-square w-full grow cursor-pointer rounded-xl border font-medium transition-all hover:z-20 hover:border-primary sm:-m-px sm:size-20 sm:rounded-2xl sm:border-2 lg:size-36 lg:rounded-3xl 2xl:size-40`}
            >
              <span className={`absolute left-1 top-1 flex size-5 items-center justify-center rounded-full text-xs sm:size-6 sm:text-sm lg:left-2 lg:top-2 lg:size-8 lg:text-base ${isToday ? 'bg-primary font-semibold text-white' : ''} ${month < 0 ? 'text-slate-400' : 'text-slate-800'}`}>
                {day}
              </span>
              {isNewMonth && (
              ...
              {sessoesDoDia.length > 0 && (
                <div className="absolute bottom-1 right-1 flex space-x-1 p-1">
                  {sessoesDoDia.slice(0, 3).map(sessao => (
                    <div key={sessao.id} className="h-1.5 w-1.5 rounded-full bg-primary" />
                  ))}
                </div>
              )}
            </div>
      ...
      <div className="sticky -top-px z-50 w-full rounded-t-2xl bg-white px-5 pt-7 sm:px-8 sm:pt-8">
        <div className="mb-4 flex w-full flex-wrap items-center justify-between gap-6">
          <div className="flex flex-wrap gap-2 sm:gap-3">
            ...
            <button onClick={onAddEventClick} type="button" className="whitespace-nowrap rounded-lg bg-primary px-3 py-1.5 text-center text-sm font-medium text-white hover:bg-primary/90 focus:outline-none focus:ring-4 focus:ring-ring sm:rounded-xl lg:px-5 lg:py-2.5">
              Adicionar Sessão
            </button>
          </div>
