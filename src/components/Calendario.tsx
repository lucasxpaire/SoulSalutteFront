...
import { ChevronLeft, ChevronRight, Plus, Edit, Trash2, Clock, User } from "lucide-react"
...
const Calendario: React.FC<CalendarioProps> = ({ sessoes, onEditSessao, onAddSessao, onSessaoUpdated }) => {
  ...
  const renderCalendarDays = () => {
    ...
    while (day <= endOfWeek(endOfMonth(currentMonth), { weekStartsOn: 0 })) {
      const cloneDay = day
          ...
          <div className="space-y-1">
            {daysSessoes.slice(0, 3).map((sessao) => (
              ...
              >
                ...
                <ContextMenuContent>
                  <ContextMenuItem onClick={() => handleEditSessao(sessao)} className="flex items-center gap-2">
                    ...
                    {daysSessoes.slice(3).map((sessao) => (
                      <div key={sessao.id} className="flex items-center justify-between p-2 border rounded">
                        ...
                        <div className="flex gap-1">
                          <Button size="sm" variant="ghost" onClick={() => handleEditSessao(sessao)}>
