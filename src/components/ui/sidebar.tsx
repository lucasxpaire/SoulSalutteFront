...
export const SidebarProvider = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, style, children, ...props }, ref) => {
    ...
    const contextValue = React.useMemo<SidebarContext>(() => ({ open, setOpen }), [open])
...
export const SidebarContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn("flex min-h-0 flex-1 flex-col gap-2 overflow-auto p-3 no-scrollbar", className)}
        {...props}
      />
    )
  },
...
export const SidebarMenuItem = React.forwardRef<HTMLLIElement, React.ComponentProps<"li"> & { tooltip?: string }>(
  ...
  return (
    ...
    >
      {React.Children.map(props.children, (child, index) => {
        ...
        return <div className="group-data-[state=closed]/sidebar-wrapper:hidden flex-1">{child}</div>
      })}
    </Comp>
  )
})
